...
@Component({
	selector: 'app-attachment-step-item',
	templateUrl: './attachment-step-item.component.html',
})
export class AttachmentStepItemComponent implements OnInit {
	...
	removeFile(event: any) {
		...
		if (!event.target.closest('.attachment-item')?.querySelector('textarea').value.length) {
			event.target.closest('app-attachment-step-item').remove();
		}
	}

	onTextChange(event: any) {		
		let item = this.curTextarea.nativeElement.closest('app-attachment-step-item');	
